...
class DirectedGraph {
private:
    map<string, map<string, int>> adjacencyList; // 邻接表表示的有向图
    set<string> nodes;                           // 所有节点集合
    ...
    string generateDotFormat() const {
        ...
        for (const auto& from : adjacencyList) {
            for (const auto& to : from.second) {
                dot << "  \"" << from.first << "\" -> \"" << to.first << "\" [label=\"" << to.second << "\"];\n";
            }
        }
...
public:
    ...
    void showDirectedGraph() {
        ...
        if (dotFile.is_open()) {
            ...
            std::string outputPath = "D:/Code/Clion/software_engineering/lab1/graph.png";
            #ifdef _WIN32
                std::string command = "dot -Tpng graph.dot -o " + outputPath;
                system(command.c_str());
            #else
                std::string command = "dot -Tpng graph.dot -o " + outputPath + " 2>/dev/null";
                system(command.c_str());
            #endif
            std::cout << "Graph image saved as " << outputPath << " (if Graphviz is installed)" << std::endl;
        } else {
    ...
    string queryBridgeWords(const string& word1, const string& word2) {
        ...
        if (adjacencyList.find(w1) != adjacencyList.end()) {
            for (const auto& edge : adjacencyList.at(w1)) {
                ...
                if (adjacencyList.find(potentialBridge) != adjacencyList.end() &&
                    adjacencyList.at(potentialBridge).find(w2) != adjacencyList.at(potentialBridge).end()) {
        ...
        string result = "The bridge words from \"" + word1 + "\" to \"" + word2 + "\" ";
    ...
    // 计算PageRank
    double calcPageRank(const string& word, double d = 0.85, int iterations = 100) {
        string w = word;
    ...
    string randomWalk() {
        ...
        while (true) {
            // 检查当前节点是否有出边
            if (adjacencyList.find(current) == adjacencyList.end() || adjacencyList[current].empty()) {
                break;
            }

            // 随机选择一个出边
            vector<pair<string, int>> edges(adjacencyList[current].begin(), adjacencyList[current].end());
        ...
        // 将结果写入文件
        ofstream outFile("D:\\Code\\Clion\\software_engineering\\lab1\\random_walk.txt");
        if (outFile.is_open()) {
            outFile << result;
            outFile.close();
            cout << "Random walk result saved to D:\\Code\\Clion\\software_engineering\\lab1\\random_walk.txt" << endl;
        } else {
